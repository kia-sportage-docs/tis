...
def is_headerish(cells):
    ...
    header_tokens = {'код', 'код 1', 'код 2', 'номер детали', 'наименование', 'кол-во', 'кол.', 'поз.', 'поз', 'позиция'}
...
def box_to_rect(box):
    ...
    return {'x': round(float(left), 1), 'y': round(float(top), 1), 'w': round(float(right - left), 1), 'h': round(float(bottom - top), 1)}
...
def find_footer_boxes(ocr_results):
    ...
    for box, text, _score in ocr_results or []:
        if FOOTER_RE.match((text or '').strip()):
            ...
            boxes.append((int(rect['x']) - FOOTER_ERASE_PAD, int(rect['y']) - FOOTER_ERASE_PAD, int(rect['x'] + rect['w']) + FOOTER_ERASE_PAD, int(rect['y'] + rect['h']) + FOOTER_ERASE_PAD))
    return boxes
...
def find_protected_boxes(ocr_results):
    ...
    for box, text, score in ocr_results or []:
        ...
        if DIRECTION_RE.match(clean):
            rect = box_to_rect(box)
            boxes.append((int(rect['x']) - 22, int(rect['y']) - 18, int(rect['x'] + rect['w']) + 42, int(rect['y'] + rect['h']) + 26))
    return boxes
...
def shift_rect(rect, crop_box):
    ...
    return {'x': round(rect['x'] - left, 1), 'y': round(rect['y'] - top, 1), 'w': rect['w'], 'h': rect['h']}
...
def locate_scheme_image(page_num: int) -> Path:
    candidates = [
        BASE / 'pages' / f'page_{page_num:03d}.jpg',
        BASE / 'pages' / f'page_{page_num:03d}.png',
        BASE / 'pages_first10' / f'page_{page_num:03d}.jpg',
        BASE / 'pages_first10' / f'page_{page_num:03d}.png',
        BASE / 'pages_pdfium' / f'page_{page_num:03d}.png',
        BASE / 'pages_pdfium' / f'page_{page_num:03d}.jpg',
    ]
    block_start = ((page_num - 1) // 10) * 10 + 1
    block_end = block_start + 9
    block_dir = BASE / f'pages_{block_start:03d}_{block_end:03d}'
    candidates.extend([
        block_dir / f'page_{page_num:03d}.jpg',
        block_dir / f'page_{page_num:03d}.png',
        block_dir / f'page-{page_num}.jpg',
        block_dir / f'page-{page_num}.png',
        block_dir / f'TIS1.5_{page_num}.jpg',
        block_dir / f'TIS1.5_{page_num}.png',
    ])
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError(f'No image found for scheme page {page_num}')
...
def main():
    ...
    (DATA_DIR / 'overrides.json').write_text('{\n  "documents": {}\n}\n', encoding='utf-8') if not (DATA_DIR / 'overrides.json').exists() else None
    ...
    for md_path in md_files:
        ...
        if not doc or doc['scheme_page'] is None:
            continue
    ...
    for page_num in sorted(scheme_pages):
        src = locate_scheme_image(page_num)
        dst = ASSETS_PAGES_DIR / f'page_{page_num:03d}.jpg'
        img, scale, original_size = build_scaled_image(src)
        tmp_full = ASSETS_PAGES_DIR / f'.tmp_full_{page_num:03d}.jpg'
    ...
    for i, doc in enumerate(docs, start=1):
        ...
        crop_tuple = (page_meta['crop_box']['left'], page_meta['crop_box']['top'], page_meta['crop_box']['right'], page_meta['crop_box']['bottom'])
        markers = find_matches(page_meta['ocr_results'], doc['parts_by_code'], crop_tuple)
        matched_codes = {m['code'] for m in markers}
        unmatched_codes = []
        for code, rows in sorted(doc['parts_by_code'].items()):
            if code not in matched_codes:
                unmatched_codes.append({'code': code, 'label': rows[0]['code'], 'rows': rows})
        result_docs.append({
            'id': doc['id'],
            'title': doc['title'],
            ...
            'stats': {'total_codes': len(doc['parts_by_code']), 'matched_codes': len(markers), 'unmatched_codes': len(unmatched_codes)},
        })
        print(f'[{i}/{len(docs)}] {doc["id"]}: matched {len(markers)}/{len(doc["parts_by_code"])} crop={page_meta["crop_box"]} size={page_meta["render_size"]}')

    catalog = {'source_base': str(BASE), 'document_count': len(result_docs), 'documents': result_docs}
